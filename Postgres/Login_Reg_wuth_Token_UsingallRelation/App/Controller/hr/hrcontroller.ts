...
import HRRepository from '../../Repositary/index'
...
const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let users = await HRRepository.hrrepositary.create(user)

    let userResponses: Hrmodel = {
      email: users.email,
      name: users.name
    }
...
const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userResponse = await HRRepository.hrrepositary.findByEmail(UserData.email)
...
const verifytoken = async (req: any, res: Response) => {
  ...
  try {
    const userResponse = await HRRepository.hrrepositary.getUser(req.id)
    let response: responseModel = {
      status: 201,
...
const deleteemp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const deletedUser = await HRRepository.hrrepositary.delete(id);
